...
left_template = cv2.resize(left_arrow, (0,0), fx=0.5, fy=0.5) 
right_template = cv2.resize(right_arrow, (0,0), fx=0.4, fy=0.4) 
star_template = cv2.resize(star, (0,0), fx=0.4, fy=0.4) 
  ...
  def image_callback(self, msg):
    ...
    if Y['m00'] > 0 and not self.stop:
        ...
        if R['m00'] > 0 and not self.stop: 
            left_res = cv2.matchTemplate(imgGray,left_gray,cv2.TM_CCOEFF_NORMED)
            right_res = cv2.matchTemplate(imgGray,right_gray,cv2.TM_CCOEFF_NORMED)
            star_res = cv2.matchTemplate(imgGray,star_gray,cv2.TM_CCOEFF_NORMED)
